 ...
 */
function buildParameters(parameters: { [key: string]: any }, encodeFields: Array<string> = []): string {
    ...
    for (const key in parameters) {
        ...
        if (value !== undefined && value !== null && value !== '') {
            let evalue = encodeFields.indexOf(key) > -1 ? encodeURI(String(value)) : String(value);
            temp.push(`${key}=${evalue}`);
        }
    }
    return temp.join('&');
}
